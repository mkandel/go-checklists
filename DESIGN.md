...
database access, unlike `internal/web`. Not yet built out beyond the
embedding scaffold (see [Open items](#open-items-deferred)).
...
- **`react`/`qwik` frontends**: backend foundations are in place
  ...
  scaffolds) but neither `web-react/` nor `web-qwik/` has an actual app
  built out yet — see [Frontend](#frontend). Both need full feature parity
  with `server` mode before they're real alternatives, not just stubs.
