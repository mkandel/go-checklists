...
func TestListChecklists_OnlyRelevantToCaller(t *testing.T) {
	...
	creatorListResp := doJSON(t, creatorClient, http.MethodGet, srv.URL+"/checklists", nil)
	...
	outsiderListResp := doJSON(t, outsiderClient, http.MethodGet, srv.URL+"/checklists", nil)
...
func TestListChecklists_StatusFilter(t *testing.T) {
	...
	badResp := doJSON(t, creatorClient, http.MethodGet, srv.URL+"/checklists?status=bogus", nil)
	defer badResp.Body.Close()
	if badResp.StatusCode != http.StatusBadRequest {
		t.Fatalf("bad status filter = %d, want 400", badResp.StatusCode)
	}

	openResp := doJSON(t, creatorClient, http.MethodGet, srv.URL+"/checklists?status=open", nil)
	...
	completeResp := doJSON(t, creatorClient, http.MethodGet, srv.URL+"/checklists?status=complete", nil)
