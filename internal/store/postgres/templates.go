...
// CreateVersion inserts t as the next version for its name (t.Version is
// computed, overwriting whatever the caller passed in), along with items.
// Call this inside Store.WithTx if concurrent version creation for the same
// name is possible, since the version-number lookup and insert must be
// serialized against each other.
func (r *TemplateRepo) CreateVersion(ctx context.Context, t *domain.Template, items []domain.TemplateItem) error {
	var latest int
	err := r.db.QueryRow(ctx,
		`SELECT version FROM templates WHERE name = $1 ORDER BY version DESC LIMIT 1 FOR UPDATE`,
		t.Name,
	).Scan(&latest)
	switch {
	case err == nil:
	...
	row := r.db.QueryRow(ctx,
		`INSERT INTO templates (name, version) VALUES ($1, $2) RETURNING id`,
		t.Name, t.Version,
	)
...
func (r *TemplateRepo) GetLatestByName(ctx context.Context, name string) (*domain.Template, []domain.TemplateItem, error) {
	row := r.db.QueryRow(ctx,
		`SELECT id, name, version FROM templates WHERE name = $1 ORDER BY version DESC LIMIT 1`, name)
...
func (r *TemplateRepo) Get(ctx context.Context, id int64) (*domain.Template, []domain.TemplateItem, error) {
	row := r.db.QueryRow(ctx, `SELECT id, name, version FROM templates WHERE id = $1`, id)
	var t domain.Template
	if err := row.Scan(&t.ID, &t.Name, &t.Version); err != nil {
...
func (r *TemplateRepo) List(ctx context.Context) ([]domain.Template, error) {
	rows, err := r.db.Query(ctx, `SELECT id, name, version FROM templates ORDER BY name, version`)
	if err != nil {
